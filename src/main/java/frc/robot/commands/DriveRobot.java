...
public class DriveRobot extends CommandBase {
  ...
  public void execute() {
    ...
    double joystickYInput = -RobotContainer.joystick.getY();//*0.7;
    double joystickZInput = RobotContainer.joystick.getZ()*0.7;
    double deadzone = 0.2;
    double turnDeadzone = 0.25;

    if (Math.abs(RobotContainer.joystick.getX()) < deadzone) {
      joystickXInput = 0;
    } else if (Math.abs(RobotContainer.joystick.getY()) < deadzone) {
      joystickYInput = 0;
    } else if (Math.abs(RobotContainer.joystick.getZ()) < turnDeadzone) {
      joystickZInput = 0;
    }
